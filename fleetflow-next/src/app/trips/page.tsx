...
import { Plus, MapPin, Scale, AlertCircle, Map as MapIcon, Search, Filter, ArrowUpDown, ChevronDown, Truck, CheckCircle2, X } from 'lucide-react';
...
const Trips: React.FC = () => {
    ...
    const openCompleteModal = (trip: Trip) => {
        ...
        setEndOdometer(String(trip.startOdometer || ''));
        setCompleteError('');
        setShowCompleteModal(true);
    };

    const handleCompleteTrip = async (e: FormEvent) => {
        e.preventDefault();
        if (!completingTrip) return;
        setCompleteError('');
        try {
            await api.patch(`/trips/${completingTrip._id}/complete`, { endOdometer: Number(endOdometer) });
            fetchData();
                                                ...
                                                <td className="px-6 py-4">
                                                    {trip.status === 'Dispatched' && (
                                                        <button
                                                            ...
                                                            className="flex items-center gap-1.5 px-3 py-1.5 bg-success/10 hover:bg-success/20 text-success font-bold rounded-xl text-xs transition-all active:scale-95"
                                                        >
                                                            <CheckCircle2 size={14} />
                                                            Finish
                                                        </button>
                                                    )}
                                                    {trip.status === 'Completed' && trip.netProfit !== undefined && (
                                                        <div className={`text-xs font-bold ${trip.netProfit >= 0 ? 'text-success' : 'text-danger'}`}>
                                                            Net: ₹{trip.netProfit.toLocaleString()}
                                                        </div>
                                                    )}
                                                </td>
                                            </motion.tr>
                        ...
                        >
                            ...
                            >
                                <div className="flex items-center justify-between mb-6">
                                    <div>
                                        <h2 className="text-xl font-bold text-slate-900">Finalize Trip</h2>
                                        <p className="text-slate-500 text-sm font-medium mt-0.5">{completingTrip.startPoint} → {completingTrip.endPoint}</p>
                                    </div>
                                    <button onClick={() => setShowCompleteModal(false)} className="p-2 rounded-full hover:bg-slate-100 transition-colors">
                                ...
                                <div className="grid grid-cols-3 gap-3 mb-6 p-4 bg-slate-50 rounded-xl">
                                    <div className="text-center">
                                        <div className="text-xs text-slate-400 font-medium mb-0.5">Revenue</div>
                                        <div className="text-sm font-bold text-success">₹{(completingTrip.revenue || 0).toLocaleString()}</div>
                                    </div>
                                    <div className="text-center border-x border-slate-200">
                                        <div className="text-xs text-slate-400 font-medium mb-0.5">Fuel Cost</div>
                                        <div className="text-sm font-bold text-slate-700">₹{(completingTrip.fuelCost || 0).toLocaleString()}</div>
                                    </div>
                                    <div className="text-center">
                                        <div className="text-xs text-slate-400 font-medium mb-0.5">Net Profit</div>
                                        <div className={`text-sm font-bold ${(completingTrip.revenue || 0) - (completingTrip.fuelCost || 0) >= 0 ? 'text-success' : 'text-danger'}`}>
                                            ₹{((completingTrip.revenue || 0) - (completingTrip.fuelCost || 0)).toLocaleString()}
                                        </div>
                                    </div>
                                </div>

                                {completeError && (
                                    <div className="mb-4 p-3 bg-danger/10 border border-danger/20 rounded-xl flex items-start gap-2 text-danger text-sm">
                                ...
                                <form onSubmit={handleCompleteTrip} className="space-y-4">
                                    <div>
                                        <label className="block text-sm font-semibold text-slate-700 mb-1">Final Odometer Reading (km)</label>
                                        {completingTrip.startOdometer !== undefined && (
                                            <p className="text-xs text-slate-400 mb-2">Started at: <span className="font-bold text-slate-600">{completingTrip.startOdometer?.toLocaleString()} km</span></p>
                                        )}
                                        <input
                                            type="number"
                                            required
                                            ...
                                            className="w-full px-4 py-2.5 border border-slate-200 rounded-xl focus:ring-2 focus:ring-success/20 outline-none font-mono"
                                            placeholder="Enter final reading"
                                            min={completingTrip.startOdometer || 0}
                                        />
                                    </div>
                                    <div className="flex gap-3 pt-2">
                                        <button type="button" onClick={() => setShowCompleteModal(false)} className="flex-1 px-4 py-3 border border-slate-200 rounded-xl font-bold text-slate-600 hover:bg-slate-50 transition-all">
                                            Cancel
                                        </button>
                                        <button type="submit" className="flex-1 px-4 py-3 bg-success text-white rounded-xl font-bold flex items-center justify-center gap-2 hover:bg-success/90 transition-all shadow-lg shadow-success/20 active:scale-95">
